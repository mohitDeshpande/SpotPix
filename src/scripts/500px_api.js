    ...
    _500px.api('/photos/search', { geo: location, page: 1, sort:"highest_rating", image_size:[2,4]}, function (response) {
        ...
        $.each(response.data.photos,function(index,photo) {
            if(!photo.nsfw) {
                ...
                var id = photo.id;
                $('div#px500 div.gallery').append("<div class='col-3 text-center gallery-img'><a data-toggle='modal' data-target='#carousel-modal' onclick='setActiveImage("+ id +")'><img class='img-fluid img-thumbnail' src='" + imageThumbnailUrl + "'></a></div>");
